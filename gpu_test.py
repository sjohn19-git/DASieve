...
# ---------------------------------------------------------------------------
# Batch PhaseNet picking for all .h5 files in h5_loc
# ---------------------------------------------------------------------------
...
if not batch_files:
    ...
    for source_file in batch_files:
        ...
        try:
            ...
            # Intercept plt.show() to save the pick plot to disk instead of displaying
            save_path = os.path.join(results_dir, f"{stem}_phasenet_picks.png")
            _orig_show = plt.show

            def _save_figure(*_, _path=save_path):
            ...
            save_picks(df_pn, file_name=source_file, author="phasenet")
            logging.info("  picks: %d  |  plot saved: %s", len(df_pn), save_path)
